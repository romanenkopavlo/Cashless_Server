...
export const refundTransaction = async(req, res) => {
    ...
    const [transaction] = await mySqlPool.query('SELECT t.id AS id_transaction, c.montant AS carte_solde, c.id AS carte_id, a.id AS affectation_id, o.id AS operation_id, t.date, t.montant AS montant_transaction, ter.id AS terminal_id, m.nom AS modele_terminal, mar.nom AS marque_terminal, c.numero AS numero_carte, o.type, u.login AS login_utilisateur, s.nom AS nom_stand FROM transactions t LEFT JOIN cartes c ON t.carte_id = c.id LEFT JOIN utilisateurs u ON c.utilisateur_id = u.id LEFT JOIN operations o ON t.operation_id = o.id LEFT JOIN affectations a ON t.affectation_id = a.id LEFT JOIN stands s ON a.stand_id = s.id LEFT JOIN terminaux ter ON t.terminal_id = ter.id LEFT JOIN modeles m ON ter.modele_id = m.id LEFT JOIN marques mar ON m.marque_id = mar.id WHERE t.id = ?', [id_transaction])
    ...
    const id_terminal = transaction[0].terminal_id
    ...
    const insertResult = await mySqlPool.query('INSERT INTO transactions (date, montant, carte_id, operation_id, affectation_id, terminal_id) VALUES (?, ?, ?, ?, ?, ?)', [formattedDate, montant, id_carte, id_operation, id_affectation, id_terminal])
